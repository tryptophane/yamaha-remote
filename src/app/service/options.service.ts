...
export class OptionsService extends AbstractService {
  ...
  enhancer(on: boolean): void {
    const command = this.generateXml(
      `<Surround><Program_Sel><Current><Enhancer>${
        on ? 'On' : 'Off'
      }</Enhancer></Current></Program_Sel></Surround>`,
      HttpMethod.PUT
    );
    this.executeCommand(command);
  }
  ...
  setBass(to: number): void {
    const command = this.generateXml(
      `<Sound_Video><Tone><Bass><Val>${to}</Val><Exp>1</Exp><Unit>dB</Unit></Bass></Tone></Sound_Video>`,
      HttpMethod.PUT
    );
    this.executeCommand(command);
  }
  ...
  setTreble(to: number): void {
    const command = this.generateXml(
      `<Sound_Video><Tone><Treble><Val>${to}</Val><Exp>1</Exp><Unit>dB</Unit></Treble></Tone></Sound_Video>`,
      HttpMethod.PUT
    );
    this.executeCommand(command);
  }
