...
export class PlaybackControlComponent {
    ...
    pause(): void {
        this.service.pause().pipe(switchMap(() => timer(0, 500)), take(6))
            .subscribe(() => this.refreshStatus());
    }

    play(): void {
        this.service.play().pipe(switchMap(() => timer(0, 500)), take(6))
            .subscribe(() => this.refreshStatus());
    }

    stop(): void {
        this.service.stop().pipe(switchMap(() => timer(0, 500)), take(6))
            .subscribe(() => this.refreshStatus());
    }

    skipNext(): void {
        this.service.skip().pipe(switchMap(() => timer(0, 500)), take(6))
            .subscribe(() => this.refreshStatus());
    }

    skipPrevious(): void {
        this.service.rewind().pipe(switchMap(() => timer(0, 500)), take(6))
            .subscribe(() => this.refreshStatus());
    }
